...
class EnvMahjong4(gym.Env):
    ...
    def __init__(self, printing=True, type=mp.Table, reward_unit=100, force_win=False, force_riichi=False, append_aval_action_obs=True):
        self.type = type
        self.t = self.type()

        if self.type == mp.Table:
            self.ACTIONS = mp.BaseAction
        elif self.type == mp_old.Table:
            self.ACTIONS = mp_old.Action
        ...
        if phase < 4:
            ...
            if self.is_deciding_riichi:
                ...
                for act in aval_actions:
                    if act.action == self.ACTIONS.Play:
                    ...
                    elif act.action != self.ACTIONS.Riichi:
                        print(act.action)
                        raise ValueError
    ...
    def step_play(self, action, playerNo, riichi=False):
        ...
        for self_action in aval_self_actions:
            if self_action.action == self.ACTIONS.Tsumo:
                can_win = True
                break
            win_action_no += 1
        ...
        if self.force_win and can_win:
            ...
            desired_action_type = self.ACTIONS.Tsumo
            self.do_not_update_hand_and_latest_tiles_this_time = False
        elif action == TSUMO:
            self.t.make_selection(win_action_no)
            desired_action_type = self.ACTIONS.Tsumo
            ...
            for act in aval_actions:
                if act.action == self.ACTIONS.Riichi:
            ...
            if self.can_riichi and action < 34 and (not self.is_deciding_riichi) and (action in self.can_riichi_tiles_id):
                ...
                if self.force_riichi:
                    for act in aval_actions:
                        if act.action == self.ACTIONS.Riichi and int(act.correspond_tiles[0].tile) == self.riichi_tile_id:
                            riichi = True
                            warnings.warn("Can richii, automatically choose to riichi !!")
                            break
                if riichi:
                    desired_action_type = self.ACTIONS.Riichi
                    ...
                    if action < 34:
                        desired_action_type = self.ACTIONS.Play
                        ...
                        desired_action_type = self.ACTIONS.AnKan
                    elif action == KAKAN:
                        desired_action_tile_id = None  # TODO: There is some simplification
                        desired_action_type = self.ACTIONS.KaKan
                    elif action == TSUMO:
                        desired_action_type = self.ACTIONS.Tsumo
                        desired_action_tile_id = None
                    elif action == PUSH:
                        desired_action_type = self.ACTIONS.KyuShuKyuHai
                        desired_action_tile_id = None
                    elif action == PASS_RIICHI:
                        desired_action_type = self.ACTIONS.Play
                        desired_action_tile_id = self.riichi_tile_id
                ...
                for act in aval_actions:
                    if act.action == desired_action_type and \
                        ...
                        if desired_action_type in [self.ACTIONS.AnKan, self.ACTIONS.KaKan]:
                ...
                if self.t.get_selected_base_action() == self.ACTIONS.Play or self.t.get_selected_base_action() == self.ACTIONS.Riichi:
                ...
                if desired_action_type == self.ACTIONS.Riichi or desired_action_type == self.ACTIONS.Play:
                    discard_tile_id = int(self.t.get_selected_action_tile().tile)
                    assert desired_action_tile_id == int(self.t.get_selected_action_tile().tile)

                    if desired_action_type == self.ACTIONS.Play:
                    ...
                    if desired_action_type == self.ACTIONS.Riichi:  # discard already done
    ...
    def step_response(self, action: int, playerNo: int):
        ...
        for response_action in aval_response_actions:
            if response_action.action in [self.ACTIONS.Ron, self.ACTIONS.ChanKan, self.ACTIONS.ChanAnKan]:
                can_win = True
                break
            win_action_no += 1
        ...
        if self.force_win and can_win:
            ...
            if action == PON:
                desired_action_type = self.ACTIONS.Pon
            elif action == MINKAN:
                desired_action_type = self.ACTIONS.Kan
            elif action in [CHILEFT, CHIMIDDLE, CHIRIGHT]:
                desired_action_type = self.ACTIONS.Chi
            elif action == PASS_RESPONSE:
                desired_action_type = self.ACTIONS.Pass
                if can_win:
                    print("见逃！！！")
            elif action == RON:
                desired_action_type = self.ACTIONS.Ron
            else:
                raise ValueError
