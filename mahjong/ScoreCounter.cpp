...
static inline bool 副露(std::string s) {
	if (s.size() == 3) return false;
	if (s.size() == 4) return s[3] != '-';
	throw runtime_error("??");
}

static inline bool 带老头(std::string s) {
	if (s[2] == 'K' || s[2] == ':' || s[2] == '|') return s[0] == '1' || s[0] == '9';
	if (s[2] == 'S') return s[0] == '1' || s[0] == '7';
	throw runtime_error("??");
}

static inline bool 纯老头(std::string s) {
	if (s[2] == 'K' || s[2] == ':' || s[2] == '|') return s[0] == '1' || s[0] == '9';
	return false;
}

static inline bool 纯绿牌(std::string s) {
	std::string first3(s.begin(), s.begin() + 3);
	return is_in({ "2sK", "3sK", "4sK", "2sS", "6sK", "8sK", "6zK", 
		"2s:", "3s:", "4s:", "6s:", "8s:","6z:" }, first3);
}

static inline bool 幺九刻子(std::string s) {
	if (s[2] == 'K' || s[2] == '|') return s[0] == '1' || s[0] == '9';
	if (s[1] == 'z') return true;
	return false;
}

static inline int is役牌对子(std::string s, Wind 自风, Wind 场风) {
...
static inline vector<std::string> remove_4(vector<std::string> strs) {
...
pair<vector<Yaku>, int> get_手役_from_complete_tiles_固定位置(
	vector<std::string> tile_group_string, Wind 自风, Wind 场风) {
	...
	// 判断单骑
	bool 单骑 = any_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s.size() == 3) return false;
		if (s.size() == 4) return s[3] == ':';
		throw runtime_error("??");
	});
	...
	// 判断门清
	bool 门清 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
	...
	// 判断有没有顺子
	bool has顺子 = any_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
	...
	// 判断是不是断幺九
	bool 断幺九 = none_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 'z') return true;
		return 带老头(s);
	});
	...
	// 统计Z一色
	bool 字一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 'z') return true;
		return false;
	});
	// 计算字一色
	...
	// 统计P清一色
	bool P清一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 'p') return true;
		return false;
	});
	bool P混一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 'p' || s[1] == 'z') return true;
		return false;
	});

	// 统计m清一色
	bool M清一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 'm') return true;
		return false;
	});
	bool M混一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 'm' || s[1] == 'z') return true;
		return false;
	});

	// 统计S清一色
	bool S清一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		if (s[1] == 's') return true;
		return false;
	});
	bool S混一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
	...
	bool 字牌刻子[7] = { false };
	bool 字牌对子[7] = { false };
	for_each(tile_group_string.begin(), tile_group_string.end(), [&字牌刻子, &字牌对子](std::string s) {
		if (s[1] == 'z') {
			if (s[2] == 'K') {
				字牌刻子[s[0] - '1'] = true;
			}
	...
	// 统计暗刻数
	int num_暗刻 = 0;
	for_each(tile_group_string.begin(), tile_group_string.end(), [&num_暗刻](std::string s) {
		if (s[2] == 'K' && s.size() == 3) {
			num_暗刻++;
		}
		if (s[2] == '|' && s[3] == '+') {
	...
	// 统计杠子数
	int num_杠子 = 0;
	for_each(tile_group_string.begin(), tile_group_string.end(), [&num_杠子](std::string s) {
		if (s[2] == '|') {
			num_杠子++;
		}
	});
	...
	bool 清老头 = true;
	bool 混老头 = true;
	for_each(tile_group_string.begin(), tile_group_string.end(), [&清老头, &混老头](std::string s) {
		if (s[1] == 'z') {
			清老头 = false;
			return;
		}
	...
	bool 纯全带幺九 = true;
	bool 混全带幺九 = true;
	for_each(tile_group_string.begin(), tile_group_string.end(), [&纯全带幺九, &混全带幺九](std::string s) {
		if (s[1] == 'z') {
			纯全带幺九 = false;
			return;
		}
	...
	平和 &= 门清;
	平和 &= none_of(tile_group_string.begin(), tile_group_string.end(), [&自风, &场风](std::string s) {
		return is役牌对子(s, 自风, 场风);
	});
	平和 &= all_of(tile_group_string.begin(), tile_group_string.end(), [&自风, &场风](std::string s) {
		if (s[2] == ':') return true;
		if (s[2] == 'S') return true;
		return false;
	});
	...
	// 判断绿一色
	bool 绿一色 = all_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		return 纯绿牌(s);
	});
	...
	bool 三色同顺 = false;
	for (int i = 1; i <= 7; ++i) {
		vector<std::string> 三色同顺tiles;
		三色同顺tiles.push_back(std::string() + char(i + '0') + "sS");
		三色同顺tiles.push_back(std::string() + char(i + '0') + "mS");
		三色同顺tiles.push_back(std::string() + char(i + '0') + "pS");
		...
		if (all_of(三色同顺tiles.begin(), 三色同顺tiles.end(),
			[&tile_group_string_no_4](std::string tiles) {return is_in(tile_group_string_no_4, tiles); })) {
			三色同顺 = true;
			break;
		}
	}
	...
	bool 三色同刻 = false;
	for (int i = 1; i <= 9; ++i) {
		vector<std::string> 三色同刻tiles;
		三色同刻tiles.push_back(std::string() + char(i + '0') + "sK");
		三色同刻tiles.push_back(std::string() + char(i + '0') + "mK");
		三色同刻tiles.push_back(std::string() + char(i + '0') + "pK");
	...
	// 判断一气通贯	
	bool 一气通贯 = false;
	vector<std::string> 一气通贯S = { "1sS", "4sS", "7sS" };
	vector<std::string> 一气通贯M = { "1sM", "4sM", "7sM" };
	vector<std::string> 一气通贯P = { "1sP", "4sP", "7sP" };
	...
	int n杯口 = 0;
	for (auto tiles : 顺子牌型)
		if (count_if(tile_group_string_no_4.begin(), tile_group_string_no_4.end(), [&tiles](std::string s) {
			return s == tiles;
		}) >= 2) {
			n杯口++;
		}
	...
	// 听牌型
	if (单骑) fu += 2;
	if (any_of(tile_group_string.begin(), tile_group_string.end(), [](std::string s) {
		// 坎张
		if (s.size() == 4)
	...
	// 雀头符
	for_each(tile_group_string.begin(), tile_group_string.end(), [&fu, &自风, &场风](std::string s) {
		// 荣和
		fu += (is役牌对子(s, 自风, 场风) * 2);
	});

	// 面子符
	for_each(tile_group_string.begin(), tile_group_string.end(), [&fu](std::string s) {
		if (s.size() == 3) {
			if (s[2] == 'K') { 
				if (幺九刻子(s)) fu += 8; 
				else fu += 4;
...
vector<pair<vector<Yaku>, int>> get_手役_from_complete_tiles(CompletedTiles ct, vector<Fulu> fulus, Tile *correspond_tile, BaseTile tsumo_tile, Wind 自风, Wind 场风)
{
	...
	std::string last_tile_string = basetile_to_string_simple(last_tile);
