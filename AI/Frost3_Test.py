...
env_test = EnvMahjong3(append_aval_action_obs=True)
max_steps = 1000
...
# agent_test0 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210802/search_beta_0_alpha_1_kldt_-1_bnt_0_eqn_1_cqla_0/data/mahjong_VLOG_BC_0.model", map_location=torch.device(device))
# agent_test1 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210802/search_beta_0_alpha_1_kldt_-1_bnt_0_eqn_1_cqla_0/data/mahjong_VLOG_BC_1.model", map_location=torch.device(device))
# agent_test2 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210802/search_beta_0_alpha_1_kldt_-1_bnt_0_eqn_1_cqla_0/data/mahjong_VLOG_BC_2.model", map_location=torch.device(device))
# agent_test3 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210802/search_beta_0_alpha_1_kldt_-1_bnt_0_eqn_1_cqla_0/data/mahjong_VLOG_BC_3.model", map_location=torch.device(device))

agent_test0 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210804/search_envid_1_beta_-1_alpha_1_kldt_10_bnt_0_dpo_0.25_eqn_1_cqla_0/data/mahjong_VLOG_BC_2.model", map_location=torch.device(device))
agent_test1 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210804/search_envid_1_beta_-1_alpha_1_kldt_10_bnt_0_dpo_0.25_eqn_1_cqla_0/data/mahjong_VLOG_BC_3.model", map_location=torch.device(device))
agent_test2 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210804/search_envid_1_beta_-1_alpha_1_kldt_10_bnt_0_dpo_0.25_eqn_1_cqla_0/data/mahjong_VLOG_BC_2.model", map_location=torch.device(device))
agent_test3 = torch.load("/mnt/c/code/snake/amlt/vlog_offlinemj_bc_20210804/search_envid_1_beta_-1_alpha_1_kldt_10_bnt_0_dpo_0.25_eqn_1_cqla_0/data/mahjong_VLOG_BC_3.model", map_location=torch.device(device))
...
while game < 10000:
    ...
    for tt in range(max_steps):
        ...
        if len(valid_actions) == 1:
            ...
            # a = np.random.choice(action_mask.shape[-1], 1, p=action_mask.reshape([-1]) / action_mask.sum())
            # ---------------------------
            ...
            if not full_obs:
                a = agents_test[curr_pid].select(env_test.get_obs(curr_pid), action_mask, greedy=False)
            else:
                a = agents_test[curr_pid].select(env_test.get_full_obs(curr_pid), action_mask, greedy=False)

            if a == 41:
                print("Player {} Riichi !!".format(curr_pid))

            if env_test.is_deciding_riichi:
                print("deciding riichi: agent select action {}".format(a))
